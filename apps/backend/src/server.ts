...
import { get, GNewsResponse, RawCse, SearchRecommendation, TRawCse, YoutubeSearchResponse } from "@packages/shared";
	...
	.get(
		...
		async ({ query }) => {
			return record("gnews.search.get", async () => {
				...
				const eff = pipe(
					get(`${GNEWS_BASE}/search`, { schema: GNewsResponse }, params),
				);
		...
		async ({ query }) => {
			return record("google.cse.get", async () => {
				try {
					...
					const raw = await run(pipe(get(CSE_ENDPOINT, { schema: RawCse }, params))) as unknown as TRawCse;
