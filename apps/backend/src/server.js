...
import { get, GNewsResponse, RawCse, SearchRecommendation, YoutubeSearchResponse, } from '@packages/shared';
 ...
 */
const ipGenerator = (_r, _s, { ip }) => ip?.address ?? 'unknown';
/**
...
function run(eff) {
    const provided = eff.pipe(Effect.provide(FetchHttpClient.layer));
 ...
 */
function normalizeWebDetection(res) {
...
const utilityRoutes = new Elysia()
    ...
    .get('/status', async () => record('status.get', async () => {
    ...
    .get('/version', async () => record('version.get', async () => {
    const appVersion = version;
    return Stringify({
        version: appVersion,
        status: 200,
    ...
    .get('/health', async () => record('health.get', () => {
...
async ({ query }) => {
    return record('google.cse.get', async () => {
        try {
            ...
            const raw = (await run(pipe(get(CSE_ENDPOINT, { schema: RawCse }, params))));
            const transformed = {
                info: {
...
async ({ onBeforeHandle, onAfterHandle, onError }) => {
    ...
    errorResponse: new Response(Stringify({
